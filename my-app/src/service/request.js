...
request.interceptors.response.use(
  (response) => {
    ...
    if (data?.code && data?.code !== 200) {    // for error
      toast.error(
        response.data.error ?? "Somthing went wrong. Please try again!" , { theme: 'colored' }
      );
  ...
  (error) => {
    ...
    toast.error(error?.response?.data?.error ?? "Somthing went wrong" , { theme: 'colored' });
